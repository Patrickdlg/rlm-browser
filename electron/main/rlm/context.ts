...
export function getSystemPrompt(options: SystemPromptOptions = {}): string {
  ...
  const examples = isSubCall
    ...
    : [
      ...
      ``,
      `### When search snippets already answer the question`,
      `Iteration 1:`,
      `\`\`\`repl`,
      `const searchTab = await openTab('https://www.google.com/search?q=population+of+tokyo+2025')`,
      `await waitForLoad(searchTab)`,
      ...
      `Iteration 2:`,
      `\`\`\`repl`,
      `// Search snippets already contain the answer — no need to open pages`,
      `const answer = await llm_query('Answer: what is the population of Tokyo?', JSON.stringify(results))`,
      `setFinal(answer)`,
      `\`\`\``,
      ``,
      `### SPA handling: LinkedIn profile extraction`,
      `LinkedIn is a SPA — \`waitForLoad()\` resolves before profile sections render. Poll \`getText()\` until substantial content appears, then pass the FULL text to \`llm_query()\` — don't manually slice sections.`,
      ``,
      `Iteration 1:`,
      `\`\`\`repl`,
      ...
      `Iteration 2:`,
      `\`\`\`repl`,
      `// LinkedIn SPA — waitForLoad is NOT enough, poll for content`,
      ...
      `}`,
      `// Pass the full text to llm_query — it handles noise, nav elements, etc.`,
      `const summary = await llm_query('Summarize this person\\'s professional background based on their LinkedIn profile', profileText)`,
      `setFinal(summary)`,
      `\`\`\``,
      ``,
      `**Key pattern:** Use \`llm_query(prompt, content)\` to analyze page content — don't manually slice substrings or log previews. \`llm_query\` can reason about noisy pages. Call \`setFinal()\` as soon as you have what you need.`,
    ]
