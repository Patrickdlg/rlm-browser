...
export function getSystemPrompt(options: SystemPromptOptions = {}): string {
  ...
  const rules = [
    ...
    ...(isSubCall ? [
      ...
      `- When \`__data\` is provided, just \`log(__data)\` to see it (output is auto-truncated). Then answer and call setFinal(). If the truncation cut off something important, use \`indexOf\` to jump to that section, or \`log(__data, 20000)\` to see more. Do NOT slice through data in small chunks across iterations.`,
  ...
  const apiSections = [
    ...
    `waitForSelector(tabId, selector, timeout?)  // Wait until CSS selector appears in DOM (default 30s) — use for SPAs`,
    '```',
    ``,
    `### DOM Introspection`,
    '```',
    ...
    '```',
    `parseHTML(html) → docHandle              // Parse HTML string into queryable doc (host-side)`,
    `parsePage(tabId, selector?) → docHandle  // getDOM + parseHTML shortcut`,
    `domQueryAll(doc, sel) → [{tag, id, className, text, attrs}]    // CSS query, all matches`,
    `domQueryOne(doc, sel) → {tag, id, className, text, attrs, innerHTML, childCount} | null`,
    `domText(doc, sel) → [string]             // textContent of all matches`,
    `freeDoc(doc)                             // Release parsed doc memory`,
    '```',
    ``,
    `### Browser Actions`,
    '```',
  ...
  apiSections.push(
    ...
    `log(message, limit?)            // Log to activity panel (default 5K chars, pass higher limit to see more)`,
  ...
  const examples = isSubCall
    ? [
      ...
      `\`\`\`repl`,
      `// Just dump it — output is auto-truncated, never worry about size`,
      `log(__data)`,
      `\`\`\``,
      `Next iteration: you've seen the content. Write your answer and call setFinal().`,
      ``,
      `If the truncated output missed something important (e.g. relevant section was past the cutoff), use \`indexOf\` to jump there:`,
      `\`\`\`repl`,
      `const idx = __data.indexOf('Experience')`,
      `log(__data.slice(idx, idx + 5000))`,
      `\`\`\``,
      `Or use \`log(__data, 20000)\` to see more (second arg overrides the truncation limit).`,
      ``,
    ...
    : [
      ...
      `\`\`\``,
      ``,
      `### Structured DOM extraction with parsePage`,
      `When you need structured data (links, tables, attributes, repeated elements), use \`parsePage\` instead of \`getText\`. It parses the DOM once and lets you query it multiple times without round-trips to the tab.`,
      ``,
      `\`\`\`repl`,
      `const tabId = tabs[0].id`,
      `const doc = await parsePage(tabId)`,
      ``,
      `// Get all links with their attributes`,
      `const links = await domQueryAll(doc, 'a[href]')`,
      `log('Found ' + links.length + ' links')`,
      ``,
      `// Get text content of specific elements`,
      `const headings = await domText(doc, 'h2')`,
      `log('Headings: ' + JSON.stringify(headings))`,
      ``,
      `// Get detailed info about a single element`,
      `const main = await domQueryOne(doc, 'main, #content, article')`,
      `log('Main content: ' + main?.tag + ' (' + main?.childCount + ' children)')`,
      ``,
      `freeDoc(doc)  // always free when done`,
      `\`\`\``,
      ``,
      `**When to use which:**`,
      `- \`getText(tabId)\` → feeding readable text to \`llm_query()\`, quick content checks. Uses \`innerText\` (clean, no hidden elements).`,
      `- \`parsePage(tabId)\` + \`domQueryAll\`/\`domText\` → structured extraction (links, tables, attributes, repeated elements). Parses once, query many times.`,
      ``,
      `### SPA handling: LinkedIn profile extraction`,
      `LinkedIn is a SPA — \`waitForLoad()\` resolves before profile sections render. After \`waitForLoad()\`, use \`waitForSelector()\` to wait for the profile content to appear, then use \`getText()\` to extract the full rendered text. LinkedIn's \`getText()\` output uses doubled section headers like "Experience\\nExperience", "Education\\nEducation", "About\\nAbout" as markers.`,
      ``,
      `Iteration 1:`,
      `\`\`\`repl`,
      `const searchTab = await openTab('https://www.google.com/search?q=person+name')`,
      `await waitForLoad(searchTab)`,
      `const results = await getSearchResults(searchTab)`,
      `log('Found ' + results.length + ' results')`,
      `log(JSON.stringify(results.slice(0, 5)))`,
      `// Look for LinkedIn profile URLs in results — best source for people queries`,
      `const linkedInResult = results.find(r => r.url.includes('linkedin.com/in/'))`,
      `if (linkedInResult) log('LinkedIn profile: ' + linkedInResult.url)`,
      `\`\`\``,
      ``,
      `Iteration 2:`,
      `\`\`\`repl`,
      `// Open LinkedIn profile — it's a SPA, so waitForLoad is NOT enough.`,
      `// LinkedIn's <main> exists before SPA renders, so waitForSelector('main') won't help.`,
      `// Poll getText() until we get substantial content (> 1000 chars).`,
      `const liTab = await openTab(linkedInResult.url)`,
      `await waitForLoad(liTab)`,
      `let profileText = await getText(liTab)`,
      `for (let i = 0; i < 10 && profileText.length < 1000; i++) {`,
      `  await sleep(2000)`,
      `  profileText = await getText(liTab)`,
      `}`,
      `log('Profile text length: ' + profileText.length)`,
      `// LinkedIn getText() returns ~20K chars but most is noise (nav, posts, endorsements).`,
      `// Sections are delimited by doubled headers: "About\\nAbout\\n", "Experience\\nExperience\\n", etc.`,
      `// Extract just About + Experience + Education:`,
      `const aboutIdx = profileText.indexOf('About\\nAbout\\n')`,
      `const skillsIdx = profileText.indexOf('Skills\\nSkills\\n')`,
      `const profileCore = profileText.slice(aboutIdx >= 0 ? aboutIdx : 0, skillsIdx >= 0 ? skillsIdx : undefined)`,
      `log('Core profile length: ' + profileCore.length)`,
      `const summary = await llm_query('Summarize this person\\'s professional background', profileCore)`,
      `setFinal(summary)`,
      `\`\`\``,
      ``,
  ...
  const sections = [
    ...
    `Use Markdown formatting (headers, bold, bullet points, links) for readable output.`,
    ``,
    ...examples,
  ]
